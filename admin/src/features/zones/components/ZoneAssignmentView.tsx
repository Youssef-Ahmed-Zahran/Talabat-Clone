...
interface ZoneAssignmentViewProps {
  form: ZoneEditorState["form"];
  ui: ZoneEditorState["ui"];
  stores: ZoneEditorState["stores"];
  drivers: ZoneEditorState["drivers"];
  actions: ZoneEditorActions;
}

export const ZoneAssignmentView: React.FC<ZoneAssignmentViewProps> = ({
  form,
  ui,
  stores,
  drivers,
                    ...
                    {(ui.activeTab === "stores"
                      ...
                      const alreadyAssigned = isStore
                        ? (stores.assignedStores || []).some(
                            (s) =>
                              String(s.storeId) === String(storeItem.id) ||
                              String(s.store?.id) === String(storeItem.id),
                          )
                        ...
                        >
                          ...
                          >
                            {alreadyAssigned ? (
                              ...
                              <>
                                <Plus size={14} className="stroke-[3]" /> Assign
                              </>
                            )}
                          </button>
                        </div>
              ...
              {(
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pb-12 animate-slide-up">
                  {(ui.activeTab === "stores"
                    ...
                    const item = isStore ? storeLink.store : driverLink.driver;
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-red-500">
                              This record is missing from the database. Remove
                              it below.
                            </p>
                          </div>
                          <button
                            onClick={() =>
                              isStore
                                ? actions.handleRemoveStore(storeLink.storeId)
